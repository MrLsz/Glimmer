...
| 5 | [运行时数据区一-程序计数器](notes/jvm/运行时数据区一-程序计数器.md) | 运行时数据区总览、程序计数器、字节码指令速查 |
| 6 | [运行时数据区二-虚拟机栈和本地方法栈](notes/jvm/运行时数据区二-虚拟机栈和本地方法栈.md) | 虚拟机栈与栈帧、本地方法栈与 JNI |
| 7 | [运行时数据区三-堆](notes/jvm/运行时数据区三-堆.md) | 堆、存活判定、垃圾收集算法、对象布局、TLAB、逃逸分析 |
| 8 | [运行时数据区四-方法区(元空间)](<notes/jvm/运行时数据区四-方法区(元空间).md>) | 方法区与常量池、永久代→元空间、元空间结构、类卸载 |
