...
加载完成后，类元数据会被放入 **方法区**，类初始化时创建的对象实例则进入 **堆**。整个过程的具体细节，将在《03 类加载机制》中展开。
...
这张图是整个 JVM 系列的「锚点图」——后续每一篇专题（运行时数据区域、类加载机制、字节码与执行引擎、垃圾回收）都会围绕其中某个子系统展开。
...
类加载器之间存在**父子层次**与**双亲委派模型**：子加载器收到加载请求时，**先委派给父加载器去尝试加载**，父加载器无法加载时才自己加载——这保证了核心类（如 `java.lang.String`）的唯一性，避免重复加载带来的混乱。这一主题的具体原理将在《03 类加载机制》中深入展开。
...
- **后续章节顺序**：运行时数据区域 → 类加载机制 → 字节码与执行引擎 → 垃圾回收 → 内存模型与并发 → 性能监控与调优实战。
