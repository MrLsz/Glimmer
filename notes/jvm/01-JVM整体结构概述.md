...
这张图是整个 JVM 系列的「锚点图」——后续每一篇专题（运行时数据区域、类加载机制（[Class 文件分析](notes/jvm/03-类加载机制一-class文件分析.md) / [类加载过程分析](notes/jvm/04-类加载机制二-类加载过程分析.md) / [双亲委派机制](notes/jvm/05-类加载机制三-双亲委派机制.md)）、字节码与执行引擎、垃圾回收）都会围绕其中某个子系统展开。
...
每个区域的具体职责与内部结构，是《02 运行时数据区域》的核心内容——那里会逐一拆解五大区域的作用、存储内容、关键参数与典型异常（`StackOverflowError` / `OutOfMemoryError`）。
...
执行引擎的具体工作原理（包括字节码解释、JIT 编译触发条件、分层编译策略）、垃圾回收的算法与各类收集器的对比，将在《04 字节码与执行引擎》和《05 垃圾回收》中分别展开。
...
使用 JNI 的代价是：**跨越 JNI 边界的调用有性能开销**，并且本地代码不受 JVM 安全管理。因此在 Java 能解决的场景下，应尽量避免频繁跨越 JNI 边界。JNI 与本地方法栈的更多细节，已在《02 运行时数据区域》的「本地方法栈」章节中讨论过。
...
- **后续章节顺序**：运行时数据区域 → 类加载机制（[Class 文件分析](notes/jvm/03-类加载机制一-class文件分析.md) → [类加载过程分析](notes/jvm/04-类加载机制二-类加载过程分析.md) → [双亲委派机制](notes/jvm/05-类加载机制三-双亲委派机制.md)）→ 字节码与执行引擎 → 垃圾回收 → 内存模型与并发 → 性能监控与调优实战。
