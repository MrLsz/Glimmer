...
> 注意：`removeFirst()` 会导致数组整体前移，时间复杂度 O(n)。对大规模数据建议使用下面的优化版。

### Swift（优化版，使用游标避免 O(n) 开销）
...
class Solution {
    func levelOrder(_ root: TreeNode?) -> [[Int]] {
        ...
        var result: [[Int]] = []
        var queue: [TreeNode] = [root]
        var queueIndex = 0  // 游标替代 removeFirst()，避免数组整体前移

        while queueIndex < queue.count {
            let levelSize = queue.count - queueIndex
            var levelValues = [Int]()
...
- (NSArray<NSArray<NSNumber *> *> *)levelOrder:(TreeNode *)root {
    ...
    NSInteger queueIndex = 0;   // 游标替代 removeFirst，避免数组整体前移

    while (queueIndex < (NSInteger)queue.count) {
        NSInteger levelSize = (NSInteger)queue.count - queueIndex;
        NSMutableArray<NSNumber *> *levelValues = [NSMutableArray arrayWithCapacity:levelSize];

        for (NSInteger i = 0; i < levelSize; i++) {
            TreeNode *node = queue[queueIndex++];
            [levelValues addObject:@(node.val)];
...
| 适用场景 | 层级处理、最短路径、连通性 | 路径搜索、拓扑问题、回溯 |
| 按层输出 | 天然支持 | 需要额外记录层级信息 |
