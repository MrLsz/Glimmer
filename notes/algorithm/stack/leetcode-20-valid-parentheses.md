...
class Solution {
    ...
    fun isValid(s: String): Boolean {
        ...
        for (bracket in s) {
            val expectedLeft = BRACKET_PAIRS[bracket]
            if (expectedLeft != null) {
                // 右括号：栈顶必须匹配
                if (stack.isEmpty() || stack.last() != expectedLeft) {
                    return false
                }
                stack.removeLast()
            } else {
...
class Solution {
    ...
    func isValid(_ s: String) -> Bool {
        ...
        for bracket in s {
            if let expectedLeft = Self.bracketPairs[bracket] {
                // 右括号：栈顶必须匹配
                if stack.isEmpty || stack.removeLast() != expectedLeft {
                    return false
                }
            } else {
                // 左括号：压入栈
...
```objc
static NSDictionary<NSString *, NSString *> *BracketPairs = nil;
...
- (BOOL)isValid:(NSString *)s {
    ...
    for (NSUInteger i = 0; i < s.length; i++) {
        NSString *bracket = [s substringWithRange:NSMakeRange(i, 1)];
        NSString *expectedLeft = BracketPairs[bracket];

        if (expectedLeft != nil) {
            // 右括号：栈顶必须匹配
            if (stack.count == 0 || ![stack.lastObject isEqualToString:expectedLeft]) {
                return NO;
            }
...
3. **映射表**：右括号到左括号的映射避免冗长的 `if-else` 判断，各语言实现方式不同（Java 用 `static final` 常量、Kotlin 用 `companion object`、Swift 用 `static let`、OC 用 `+initialize`）
