...
| 本篇范围 | 上篇：输入事件的读取。从「手指按下」到「InputReader 把事件交给 InputDispatcher」为止 |
...
- 上篇（本篇）：输入事件的读取。内核把事件写进设备节点后，InputReader 如何通过 EventHub 把它读出来、加工成 Android 认识的输入事件。
- 中篇：输入事件的派发。InputDispatcher 如何根据 WMS 提供的窗口信息找到目标窗口，通过 InputChannel 把事件塞进窗口的接收管道。
- 下篇：输入事件的应用层处理。窗口侧 ViewRootImpl 如何从管道取出事件，沿着 View 树完成 dispatchEvent / processEvent，最终让控件响应。

本篇只讲「读取」这一段，终点是 InputReader 把加工好的事件 flush 给 InputDispatcher，派发的事留给中篇。全文拿一次手指点击当例子，遇到哪一段都能对应上：点下去的那一下，代码到底在跑什么。
...
因为 `InputDispatcher` 继承了 `InputListenerInterface`，所以 `mInnerListener` 就是 InputDispatcher。`flush()` 之后，事件就正式落到了 InputDispatcher 手里，上篇到此为止，派发的事交给中篇。
...
到这里，一次点击完成了「读取」的全部旅程：它从内核里的一串裸数据，变成了一个 InputDispatcher 认识的、`ACTION_DOWN` 的 MotionEvent。至于 InputDispatcher 接下来怎么找窗口、怎么写通道，那是中篇「输入事件的派发」的内容。
