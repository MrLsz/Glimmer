...
| 本篇范围 | 第二篇：派发过程。从「InputDispatcher 收到事件」到「事件写入窗口的 InputChannel 管道」为止 |
| 主线 | 关键数据结构 → 派发主循环 → 寻找目标窗口 → 派发执行 → ANR → InputChannel 通道 → 窗口信息同步 → 点击派发全流程 |

## 导读

第一篇停在 InputReader 把加工好的 MotionEvent flush 给 InputDispatcher。那一串从内核 `/dev/input/eventX` 里读出来的裸数据，到这里已经是一个带语义的 `ACTION_DOWN` 事件了。但事件有了，该给谁还没定。
...
系列三篇的划分在第一篇导读里交代过，这里再对齐一次：

- 第一篇：获取过程。内核写设备节点 → EventHub → InputReader → flush 给 InputDispatcher。
- 第二篇（本篇）：派发过程。InputDispatcher 找目标窗口 → 写 InputChannel → 等 FINISHED 回执。
- 第三篇：应用层处理。窗口侧 ViewRootImpl 从管道取出事件，沿 View 树完成 dispatchEvent / processEvent。

和第一篇一样，全文拿一次点击当例子。第一篇的终点（InputDispatcher 收到 `ACTION_DOWN`）正是本篇的起点。
...
InputDispatcher 是派发阶段的绝对主角。第一篇讲过，它运行在一条独立的 Native 线程里，优先级 `PRIORITY_URGENT_DISPLAY`。职责一句话：从 InputReader 手里接过事件，根据 WMS 喂过来的窗口信息找到该收事件的窗口，再把事件写进窗口的 InputChannel。
...
这跟第一篇 InputReader 的 `loopOnce` 是对称的一对。`dispatchOnce` 一轮做三件事：
...
承接第一篇的「一次点击」。第一篇终点是 InputReader 把 `ACTION_DOWN` 的 MotionEvent flush 给 InputDispatcher。现在从这个点继续，看这一个事件在派发阶段走完的每一步。
...
6. 窗口消费：桌面进程主线程的 Looper 被唤醒，InputConsumer recv 到事件，InputEventReceiver 把它分发出去，交给 ViewRootImpl 处理（这段是第三篇的内容）。
...
到这里，一个 `ACTION_DOWN` 完成了「派发」的全部旅程：从 inboundQueue 里的一条记录，变成了窗口进程主线程即将处理的一个输入事件。至于窗口侧怎么从管道取出、怎么沿 View 树分发，那是第三篇「应用层处理」的内容。
