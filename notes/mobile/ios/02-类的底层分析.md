...
先看一张 OC 类的数据结构总览图，把本章核心结构（isa / superclass / cache / bits / ro / rw）和父子继承链串起来——后续小节再逐字段拆解：

![类数据结构总览：核心结构与父子继承](images/cls-struct.png)

**图示说明**（配合图看）：

- **objc_object**：对象之根，唯一成员 `isa_t isa`；
- **objc_class**（继承 objc_object）：四个核心字段 `isa` / `superclass` / `cache` / `bits`，共 40 字节；
- **父子继承**：每个 `objc_class` 的 `superclass` 指向父类，沿链直到根类 NSObject 的 `superclass = nil`；
- **isa 链**：实例 → isa → 类 → isa → 元类（图中省略，详见 1.10 节）；
- **cache → buckets**：方法缓存，桶数组里每个 `bucket_t` 存 `SEL → IMP`；
- **bits → rw → ro / methods**：类数据入口指向 `class_rw_t`，rw 通过 ro 指针持有只读数据、并持有方法/属性/协议数组；
- **method_t**：方法最小单元，SEL + types + IMP 三要素。
